...
import { getBaseFreightTermsList,saveBaseFreightTerms } from "@/api/dynamic_configuration_platform/basic_manage/base_freight_terms_service";
...
const BaseFreightTerms : React.FC = () => {

    // 运费条款数据
    const [baseFreightTermsList, setBaseFreightTermsList] = useState([] as BaseFreightTermsItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseFreightTermsData = await getBaseFreightTermsList();
            // 设置运费条款台账数据
            setBaseFreightTermsList([...baseFreightTermsData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseFreightTermsItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseFreightTermsItemProps) => {
        const newData = baseFreightTermsList.filter((item) => `${item.Code}` === `${record.Code}`);
    ...
    const initFormData = {} as BaseFreightTermsItemProps;
    const [formData, setFormData] = useState<BaseFreightTermsItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseFreightTerms(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseFreightTermsItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_freight_terms' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_freight_terms' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_freight_terms' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 运费条款
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseFreightTermsItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Code}`}
                    showSorterTooltip={false}
                    dataSource={baseFreightTermsList}
                    loading={baseFreightTermsList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseFreightTerms;
