...
import { getBusinessPartnerList,saveBusinessPartner } from "@/api/dynamic_configuration_platform/basic_manage/business_partner_service";
...
const BusinessPartner : React.FC = () => {
    ...
    // 合作伙伴数据
    const [businessPartnerList, setBusinessPartnerList] = useState([] as BusinessPartnerItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const businessPartnerData = await getBusinessPartnerList();
            // 设置合作伙伴台账数据
            setBusinessPartnerList([...businessPartnerData]);
        };
        getData();
    }, []);
    ...
    const [formData, setFormData] = useState<BusinessPartnerItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBusinessPartner(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BusinessPartnerItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
                ...
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 合作伙伴
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BusinessPartnerItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Id}`}
                    showSorterTooltip={false}
                    dataSource={businessPartnerList}
                    loading={businessPartnerList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
