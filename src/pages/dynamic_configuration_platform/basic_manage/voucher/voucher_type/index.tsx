...
import { VoucherTypeItemProps } from "@/types/dynamic_configuration_platform/basic_manage/voucher_type";
import { getVoucherTypeList,saveVoucherType } from "@/api/dynamic_configuration_platform/basic_manage/voucher_type_service";
...
const VoucherType : React.FC = () => {

    // 凭证类型数据
    const [voucherTypeList, setVoucherTypeList] = useState([] as VoucherTypeItemProps[]);
    ...
    const handleDelete = (record:VoucherTypeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:VoucherTypeItemProps) => {
        const newData = voucherTypeList.filter((item) => `${item.TypeCode}` === `${record.TypeCode}`);
    ...
    const initFormData = {} as VoucherTypeItemProps;
    const [formData, setFormData] = useState<VoucherTypeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveVoucherType(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<VoucherTypeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='voucher_type' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='voucher_type' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='voucher_type' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 凭证类型
                            <Tooltip
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>凭证类型</b></span>根据经济业务的不同性质和处理方式对记账凭证进行分类的一种方法。不同的凭证类型反映了企业日常运营中的各种财务活动，有助于更好地组织、管理和审计会计信息。
                                            </li>
                                        </ol>
                                    </div>
                                }
