...
interface DetailModalProps {
    ...
    modalFlag: 'add' | 'edit';
    saving: boolean;
    formData: Partial<SysLoginLogItemProps>;
    onCancel: () => void;
...
const DetailModal: React.FC<DetailModalProps> = ({
    ...
    return (
        <Modal 
            open={open} 
            title={modalFlag === 'add' ? "新增登录日志" : "编辑登录日志"}
            onCancel={onCancel}
            width={'95%'}
            height={'95%'}
            destroyOnClose={true}
        ...
        >
            <Form {...formItemLayout} style={{ maxWidth: 600 }} initialValues={formData} disabled={saving} onFinish={onOk}>
                        ...
                        <Form.Item label="操作" name="Operation" rules={[{ required: true, message: '' }]}>
                            <Input onChange={onChange} />
                        </Form.Item>
                        <Form.Item label="IP" name="IpAddress" >
                            <Input onChange={onChange} />
                        </Form.Item>
                        <Form.Item label="登录时间"  rules={[{ required: true, message: '' }]}>
                            <DatePicker name="LoginTime" style={{ display: 'block' }} defaultValue={dayjs(formData.LoginTime)}  onChange={(_, dateStrings) => {onDateChange("LoginTime", dateStrings || '') }} />
                        </Form.Item>
                        <Form.Item label="登录设备" name="LoginDevice" >
                            <Input onChange={onChange} />
                        </Form.Item>
