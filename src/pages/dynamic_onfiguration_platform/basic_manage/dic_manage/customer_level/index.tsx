...
import { CustomerLevelItemProps } from "@/types/dynamic_onfiguration_platform/basic_manage/customer_level";
import { getCustomerLevelList,saveCustomerLevel } from "@/api/dynamic_onfiguration_platform/basic_manage/customer_level_service";
...
const CustomerLevel : React.FC = () => {
    ...
    // 客户级别数据
    const [customerLevelList, setCustomerLevelList] = useState([] as CustomerLevelItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const customerLevelData = await getCustomerLevelList();
            // 设置客户级别台账数据
            setCustomerLevelList([...customerLevelData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:CustomerLevelItemProps) => {
        alert(record);
    };
    const handleEdit = (record:CustomerLevelItemProps) => {
        const newData = customerLevelList.filter((item) => `${item.Code}` === `${record.Code}`);
    ...
    const initFormData = {} as CustomerLevelItemProps;
    const [formData, setFormData] = useState<CustomerLevelItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveCustomerLevel(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<CustomerLevelItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='customer_level' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='customer_level' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='customer_level' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 客户级别
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<CustomerLevelItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Code}`}
                    showSorterTooltip={false}
                    dataSource={customerLevelList}
                    loading={customerLevelList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
