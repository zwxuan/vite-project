...
import AssignEmployeeModal from './assign_employee_modal';
...
const RolePermission: React.FC = () => {
    ...
    // 分配用户取消
    const handleAssignEmployeeCancel = () => {
        setOpenAssignEmployee(false);
    };
    // 保存分配用户
    const handleAssignEmployeeOk = async () => {
        alert('保存分配用户');
    };
    return (
        <div>
            <AssignEmployeeModal
                ...
                saving={saving}
                roleCodes={roleCodes}
                onCancel={handleAssignEmployeeCancel}
                onOk={handleAssignEmployeeOk}
            />
            <Row gutter={2}>
                <Col span={12}>
