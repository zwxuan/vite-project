...
import { BaseContainerTeuItemProps } from "@/types/dynamic_onfiguration_platform/basic_manage/base_container_teu";
import { getBaseContainerTeuList,saveBaseContainerTeu } from "@/api/dynamic_onfiguration_platform/basic_manage/base_container_teu_service";
...
const BaseContainerTeu : React.FC = () => {

    // 箱型 TEU数据
    const [baseContainerTeuList, setBaseContainerTeuList] = useState([] as BaseContainerTeuItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseContainerTeuData = await getBaseContainerTeuList();
            // 设置箱型 TEU台账数据
            setBaseContainerTeuList([...baseContainerTeuData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseContainerTeuItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseContainerTeuItemProps) => {
        const newData = baseContainerTeuList.filter((item) => `${item.ContainerCode}` === `${record.ContainerCode}`);
    ...
    const initFormData = {} as BaseContainerTeuItemProps;
    const [formData, setFormData] = useState<BaseContainerTeuItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseContainerTeu(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseContainerTeuItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_container_teu' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_container_teu' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_container_teu' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 箱型 TEU
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseContainerTeuItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.ContainerCode}`}
                    showSorterTooltip={false}
                    dataSource={baseContainerTeuList}
                    loading={baseContainerTeuList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseContainerTeu;
