...
interface DetailModalProps {
    ...
    modalFlag: 'add' | 'edit';
    saving: boolean;
    formData: Partial<BaseBankBranchItemProps>;
    onCancel: () => void;
...
const DetailModal: React.FC<DetailModalProps> = ({
    ...
    return (
        <Modal 
            open={open} 
            title={modalFlag === 'add' ? "新增银行网点" : "编辑银行网点"}
            onCancel={onCancel}
            destroyOnClose={true}
            maskClosable={false}
            closable={!saving}
        ...
        >
            <Form {...formItemLayout} style={{ maxWidth: 600 }} initialValues={formData} disabled={saving} onFinish={onOk}>
                        <Form.Item label="银行网点编码" name="BankBranchCode" rules={[{ required: true, message: '' }]}>
                            <Input onChange={onChange} />
                        </Form.Item>
                        <Form.Item label="银行网点名称" name="BankBranchName" rules={[{ required: true, message: '' }]}>
                            <Input onChange={onChange} />
                        </Form.Item>
                        <Form.Item label="银行类别" name="BankType" rules={[{ required: true, message: '' }]}>
                            <Input onChange={onChange} />
                        </Form.Item>
                        <Form.Item label="联行号" name="BankUnionCode" >
                            <Input onChange={onChange} />
                        </Form.Item>
                        ...
                        <Form.Item label="swift码" name="SwiftCode" >
                            <Input onChange={onChange} />
                        </Form.Item>
                        ...
                        <Form.Item label="最后修改人" name="LastUpdateUser" >
                            <label>{}</label>
                        </Form.Item>
                        <Form.Item label="最后修改时间" name="LastUpdateTime" >
                            <label>{}</label>
                        </Form.Item>
