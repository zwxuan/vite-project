...
export function useTableOperations<T extends TableItem>({
    ...
    const handleDelete = (record: T) => {
        const newData = dataList.filter(item => item.SeqNo !== record.SeqNo);
        setDataList(newData);
    };

    const handleEdit = (record: T) => {
        setEditingKey(record.SeqNo?.toString() || '');
    };
    ...
    const handleSave = async (record: T) => {
        try {
            ...
            const index = newData.findIndex(item => record.SeqNo === item.SeqNo);
            if (index > -1) {
