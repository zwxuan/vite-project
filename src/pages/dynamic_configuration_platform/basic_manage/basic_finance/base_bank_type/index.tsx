...
import { getBaseBankTypeList,saveBaseBankType } from "@/api/dynamic_configuration_platform/basic_manage/base_bank_type_service";
...
const BaseBankType : React.FC = () => {

    // 银行类别数据
    const [baseBankTypeList, setBaseBankTypeList] = useState([] as BaseBankTypeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseBankTypeData = await getBaseBankTypeList();
            // 设置银行类别台账数据
            setBaseBankTypeList([...baseBankTypeData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseBankTypeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseBankTypeItemProps) => {
        const newData = baseBankTypeList.filter((item) => `${item.BankTypeCode}` === `${record.BankTypeCode}`);
    ...
    const initFormData = {} as BaseBankTypeItemProps;
    const [formData, setFormData] = useState<BaseBankTypeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseBankType(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseBankTypeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_bank_type' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_bank_type' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_bank_type' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 银行类别
                            <Tooltip
                                title={
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseBankTypeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.BankTypeCode}`}
                    showSorterTooltip={false}
                    dataSource={baseBankTypeList}
                    loading={baseBankTypeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
