...
import { getRoleManageList,saveRoleManage } from "@/api/dynamic_configuration_platform/identity/role_manage_service";
...
const RoleManage : React.FC = () => {
    ...
    const handleDelete = (record:RoleManageItemProps) => {
        alert(record);
    };
    const handleEdit = (record:RoleManageItemProps) => {
        const newData = roleManageList.filter((item) => `${item.RoleCode}` === `${record.RoleCode}`);
    ...
    const handleAdd = () => {
        navigate('/role/role_manage/detail');
    };

    const initFormData = {} as RoleManageItemProps;
    const [formData, setFormData] = useState<RoleManageItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveRoleManage(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='role_manage' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='role_manage' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='role_manage' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 角色管理
                            <Tooltip
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>说明</b></span>角色是为完成某种特定的业务职能指定的一组权限集合，将角色赋给一个用户，这个用户就拥有了这个角色中的所有权限；系统角色是系统预置的，不允许删除，但可以编辑角色名称、调整关联的功能。
                                            </li>
                                        </ol>
                                    </div>
                                }
