...
import { BaseRailwayPortItemProps } from "@/types/dynamic_onfiguration_platform/basic_manage/base_railway_port";
import { getBaseRailwayPortList,saveBaseRailwayPort } from "@/api/dynamic_onfiguration_platform/basic_manage/base_railway_port_service";
...
const BaseRailwayPort : React.FC = () => {

    // 铁运港口数据
    const [baseRailwayPortList, setBaseRailwayPortList] = useState([] as BaseRailwayPortItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseRailwayPortData = await getBaseRailwayPortList();
            // 设置铁运港口台账数据
            setBaseRailwayPortList([...baseRailwayPortData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseRailwayPortItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseRailwayPortItemProps) => {
        const newData = baseRailwayPortList.filter((item) => ` === `);
    ...
    const initFormData = {} as BaseRailwayPortItemProps;
    const [formData, setFormData] = useState<BaseRailwayPortItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseRailwayPort(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseRailwayPortItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_railway_port' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_railway_port' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_railway_port' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 铁运港口
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseRailwayPortItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `{record.StationId}`}
                    showSorterTooltip={false}
                    dataSource={baseRailwayPortList}
                    loading={baseRailwayPortList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseRailwayPort;
