...
import { BaseBusinessTypeItemProps } from "@/types/dynamic_configuration_platform/basic_manage/base_business_type";
import { getBaseBusinessTypeList,saveBaseBusinessType } from "@/api/dynamic_configuration_platform/basic_manage/base_business_type_service";
...
const BaseBusinessType : React.FC = () => {

    // 业务类型数据
    const [baseBusinessTypeList, setBaseBusinessTypeList] = useState([] as BaseBusinessTypeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseBusinessTypeData = await getBaseBusinessTypeList();
            // 设置业务类型台账数据
            setBaseBusinessTypeList([...baseBusinessTypeData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseBusinessTypeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseBusinessTypeItemProps) => {
        const newData = baseBusinessTypeList.filter((item) => `${item.BusinessTypeCode}` === `${record.BusinessTypeCode}`);
    ...
    const initFormData = {} as BaseBusinessTypeItemProps;
    const [formData, setFormData] = useState<BaseBusinessTypeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseBusinessType(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseBusinessTypeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_business_type' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_business_type' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_business_type' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 业务类型
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseBusinessTypeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.BusinessTypeCode}`}
                    showSorterTooltip={false}
                    dataSource={baseBusinessTypeList}
                    loading={baseBusinessTypeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseBusinessType;
