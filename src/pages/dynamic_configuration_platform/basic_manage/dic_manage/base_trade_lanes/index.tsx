...
import { getBaseTradeLanesList,saveBaseTradeLanes } from "@/api/dynamic_configuration_platform/basic_manage/base_trade_lanes_service";
...
const BaseTradeLanes : React.FC = () => {

    // 航线归类数据
    const [baseTradeLanesList, setBaseTradeLanesList] = useState([] as BaseTradeLanesItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseTradeLanesData = await getBaseTradeLanesList();
            // 设置航线归类台账数据
            setBaseTradeLanesList([...baseTradeLanesData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseTradeLanesItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseTradeLanesItemProps) => {
        const newData = baseTradeLanesList.filter((item) => ` === `);
    ...
    const initFormData = {} as BaseTradeLanesItemProps;
    const [formData, setFormData] = useState<BaseTradeLanesItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseTradeLanes(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseTradeLanesItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_trade_lanes_' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_trade_lanes_' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_trade_lanes_' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 航线
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseTradeLanesItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.LaneCode}`}
                    showSorterTooltip={false}
                    dataSource={baseTradeLanesList}
                    loading={baseTradeLanesList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
