...
import { getFunctionPermissionByUserList,saveFunctionPermissionByUser } from "@/api/dynamic_configuration_platform/identity/function_permission_by_user_service";
...
const FunctionPermissionByUser : React.FC = () => {

    // 功能权限按用户数据
    const [functionPermissionByUserList, setFunctionPermissionByUserList] = useState([] as FunctionPermissionByUserItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const functionPermissionByUserData = await getFunctionPermissionByUserList();
            // 设置功能权限按用户台账数据
            setFunctionPermissionByUserList([...functionPermissionByUserData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:FunctionPermissionByUserItemProps) => {
        alert(record);
    };
    const handleEdit = (record:FunctionPermissionByUserItemProps) => {
        const newData = functionPermissionByUserList.filter((item) => `${item.UserCode}` === `${record.UserCode}`);
    ...
    const initFormData = {} as FunctionPermissionByUserItemProps;
    const [formData, setFormData] = useState<FunctionPermissionByUserItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveFunctionPermissionByUser(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<FunctionPermissionByUserItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
                ...
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 功能权限按用户
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<FunctionPermissionByUserItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.UserCode}-${record.PositionName}-${record.RoleName}-${record.MenuFullPath}-${record.FunctionName}`}
                    showSorterTooltip={false}
                    dataSource={functionPermissionByUserList}
                    loading={functionPermissionByUserList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default FunctionPermissionByUser;
