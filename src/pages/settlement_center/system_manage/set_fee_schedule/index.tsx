...
import { SetFeeScheduleItemProps } from "@/types/settlement_center/system_manage/set_fee_schedule";
import { getSetFeeScheduleList,saveSetFeeSchedule } from "@/api/settlement_center/system_manage/set_fee_schedule_service";
...
const SetFeeSchedule : React.FC = () => {

    // 设置费用方案数据
    const [setFeeScheduleList, setSetFeeScheduleList] = useState([] as SetFeeScheduleItemProps[]);
    ...
    const handleDelete = (record:SetFeeScheduleItemProps) => {
        alert(record);
    };
    ...
    const initFormData = {} as SetFeeScheduleItemProps;
    const [formData, setFormData] = useState<SetFeeScheduleItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveSetFeeSchedule(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<SetFeeScheduleItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='set_fee_schedule' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='set_fee_schedule' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='set_fee_schedule' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 设置费用方案
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<SetFeeScheduleItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.BusinessType}`}
                    showSorterTooltip={false}
                    dataSource={setFeeScheduleList}
                    loading={setFeeScheduleList.length === 0}
                    pagination={
                        {
                            size:'small',
