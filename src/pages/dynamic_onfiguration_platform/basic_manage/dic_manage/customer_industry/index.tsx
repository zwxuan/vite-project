...
import { CustomerIndustryItemProps } from "@/types/dynamic_onfiguration_platform/basic_manage/customer_industry";
import { getCustomerIndustryList,saveCustomerIndustry } from "@/api/dynamic_onfiguration_platform/basic_manage/customer_industry_service";
...
const CustomerIndustry : React.FC = () => {

    // 客户行业数据
    const [customerIndustryList, setCustomerIndustryList] = useState([] as CustomerIndustryItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const customerIndustryData = await getCustomerIndustryList();
            // 设置客户行业台账数据
            setCustomerIndustryList([...customerIndustryData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:CustomerIndustryItemProps) => {
        alert(record);
    };
    const handleEdit = (record:CustomerIndustryItemProps) => {
        const newData = customerIndustryList.filter((item) => `${item.Code}` === `${record.Code}`);
    ...
    const initFormData = {} as CustomerIndustryItemProps;
    const [formData, setFormData] = useState<CustomerIndustryItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveCustomerIndustry(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<CustomerIndustryItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='customer_industry' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='customer_industry' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='customer_industry' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 客户行业
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<CustomerIndustryItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Code}`}
                    showSorterTooltip={false}
                    dataSource={customerIndustryList}
                    loading={customerIndustryList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
