...
import { getBaseCompanyNatureList,saveBaseCompanyNature } from "@/api/dynamic_configuration_platform/basic_manage/base_company_nature_service";
...
const BaseCompanyNature : React.FC = () => {

    // 企业性质数据
    const [baseCompanyNatureList, setBaseCompanyNatureList] = useState([] as BaseCompanyNatureItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseCompanyNatureData = await getBaseCompanyNatureList();
            // 设置企业性质台账数据
            setBaseCompanyNatureList([...baseCompanyNatureData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseCompanyNatureItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseCompanyNatureItemProps) => {
        const newData = baseCompanyNatureList.filter((item) => `${item.CompanyNatureCode}` === `${record.CompanyNatureCode}`);
    ...
    const initFormData = {} as BaseCompanyNatureItemProps;
    const [formData, setFormData] = useState<BaseCompanyNatureItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseCompanyNature(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseCompanyNatureItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_company_nature' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_company_nature' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_company_nature' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 企业性质
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseCompanyNatureItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.CompanyNatureCode}`}
                    showSorterTooltip={false}
                    dataSource={baseCompanyNatureList}
                    loading={baseCompanyNatureList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseCompanyNature;
