...
import { getSysExceptionLogList,saveSysExceptionLog } from "@/api/dynamic_configuration_platform/system_manage/sys_exception_log_service";
...
const SysExceptionLog : React.FC = () => {

    // 异常日志数据
    const [sysExceptionLogList, setSysExceptionLogList] = useState([] as SysExceptionLogItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const sysExceptionLogData = await getSysExceptionLogList();
            // 设置异常日志台账数据
            setSysExceptionLogList([...sysExceptionLogData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:SysExceptionLogItemProps) => {
        alert(record);
    };
    const handleEdit = (record:SysExceptionLogItemProps) => {
        const newData = sysExceptionLogList.filter((item) => `${item.UserCode}` === `${record.UserCode}`);
    ...
    const initFormData = {} as SysExceptionLogItemProps;
    const [formData, setFormData] = useState<SysExceptionLogItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveSysExceptionLog(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<SysExceptionLogItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
                ...
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 异常日志
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<SysExceptionLogItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.UserCode}`}
                    showSorterTooltip={false}
                    dataSource={sysExceptionLogList}
                    loading={sysExceptionLogList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default SysExceptionLog;
