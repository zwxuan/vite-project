...
import { BaseCargoTypeItemProps } from "@/types/dynamic_configuration_platform/basic_manage/base_cargo_type";
import { getBaseCargoTypeList,saveBaseCargoType } from "@/api/dynamic_configuration_platform/basic_manage/base_cargo_type_service";
...
const BaseCargoType : React.FC = () => {

    // 货物类型数据
    const [baseCargoTypeList, setBaseCargoTypeList] = useState([] as BaseCargoTypeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseCargoTypeData = await getBaseCargoTypeList();
            // 设置货物类型台账数据
            setBaseCargoTypeList([...baseCargoTypeData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseCargoTypeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseCargoTypeItemProps) => {
        const newData = baseCargoTypeList.filter((item) => `${item.Id}` === `${record.Id}`);
    ...
    const initFormData = {} as BaseCargoTypeItemProps;
    const [formData, setFormData] = useState<BaseCargoTypeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseCargoType(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseCargoTypeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_cargo_type' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_cargo_type' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_cargo_type' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 货物类型
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseCargoTypeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Id}`}
                    showSorterTooltip={false}
                    dataSource={baseCargoTypeList}
                    loading={baseCargoTypeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
