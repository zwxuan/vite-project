...
import { getSysLoginLogList,saveSysLoginLog } from "@/api/dynamic_configuration_platform/system_manage/sys_login_log_service";
...
const SysLoginLog : React.FC = () => {

    // 登录日志数据
    const [sysLoginLogList, setSysLoginLogList] = useState([] as SysLoginLogItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const sysLoginLogData = await getSysLoginLogList();
            // 设置登录日志台账数据
            setSysLoginLogList([...sysLoginLogData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:SysLoginLogItemProps) => {
        alert(record);
    };
    const handleEdit = (record:SysLoginLogItemProps) => {
        const newData = sysLoginLogList.filter((item) => `${item.UserCode}` === `${record.UserCode}`);
    ...
    const initFormData = {} as SysLoginLogItemProps;
    const [formData, setFormData] = useState<SysLoginLogItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveSysLoginLog(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<SysLoginLogItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
                ...
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 登录日志
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<SysLoginLogItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.UserCode}`}
                    showSorterTooltip={false}
                    dataSource={sysLoginLogList}
                    loading={sysLoginLogList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
