...
import { getBaseCompanySizeList,saveBaseCompanySize } from "@/api/dynamic_configuration_platform/basic_manage/base_company_size_service";
...
const BaseCompanySize : React.FC = () => {

    // 企业规模数据
    const [baseCompanySizeList, setBaseCompanySizeList] = useState([] as BaseCompanySizeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseCompanySizeData = await getBaseCompanySizeList();
            // 设置企业规模台账数据
            setBaseCompanySizeList([...baseCompanySizeData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseCompanySizeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseCompanySizeItemProps) => {
        const newData = baseCompanySizeList.filter((item) => `${item.CompanySizeCode}` === `${record.CompanySizeCode}`);
    ...
    const initFormData = {} as BaseCompanySizeItemProps;
    const [formData, setFormData] = useState<BaseCompanySizeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseCompanySize(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseCompanySizeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_company_size' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_company_size' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_company_size' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 企业规模
                            <Tooltip
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>说明</b></span>企业规模一般分为特大型、大型、中型、小型、微型。具体标准根据企业从业人员、营业收入、资产总额等指标，结合行业特点制定。
                                            </li>
                                        </ol>
                                    </div>
                                }
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseCompanySizeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.CompanySizeCode}`}
                    showSorterTooltip={false}
                    dataSource={baseCompanySizeList}
                    loading={baseCompanySizeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseCompanySize;
