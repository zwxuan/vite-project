...
import { getBaseAccountPurposeList,saveBaseAccountPurpose } from "@/api/dynamic_configuration_platform/basic_manage/base_account_purpose_service";
...
const BaseAccountPurpose : React.FC = () => {

    // 账户用途数据
    const [baseAccountPurposeList, setBaseAccountPurposeList] = useState([] as BaseAccountPurposeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseAccountPurposeData = await getBaseAccountPurposeList();
            // 设置账户用途台账数据
            setBaseAccountPurposeList([...baseAccountPurposeData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseAccountPurposeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseAccountPurposeItemProps) => {
        const newData = baseAccountPurposeList.filter((item) => `${item.AccountPurposeCode}` === `${record.AccountPurposeCode}`);
    ...
    const initFormData = {} as BaseAccountPurposeItemProps;
    const [formData, setFormData] = useState<BaseAccountPurposeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseAccountPurpose(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseAccountPurposeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_account_purpose' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_account_purpose' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_account_purpose' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 账户用途
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseAccountPurposeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.AccountPurposeCode}`}
                    showSorterTooltip={false}
                    dataSource={baseAccountPurposeList}
                    loading={baseAccountPurposeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseAccountPurpose;
