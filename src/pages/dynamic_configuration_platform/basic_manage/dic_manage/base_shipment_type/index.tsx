...
import { getBaseShipmentTypeList,saveBaseShipmentType } from "@/api/dynamic_configuration_platform/basic_manage/base_shipment_type_service";
...
const BaseShipmentType : React.FC = () => {

    // 出运类型数据
    const [baseShipmentTypeList, setBaseShipmentTypeList] = useState([] as BaseShipmentTypeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseShipmentTypeData = await getBaseShipmentTypeList();
            // 设置出运类型台账数据
            setBaseShipmentTypeList([...baseShipmentTypeData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseShipmentTypeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseShipmentTypeItemProps) => {
        const newData = baseShipmentTypeList.filter((item) => `${item.ShipmentTypeId}` === `${record.ShipmentTypeId}`);
    ...
    const initFormData = {} as BaseShipmentTypeItemProps;
    const [formData, setFormData] = useState<BaseShipmentTypeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseShipmentType(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseShipmentTypeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_shipment_type' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_shipment_type' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_shipment_type' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 出运类型
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseShipmentTypeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.ShipmentTypeId}`}
                    showSorterTooltip={false}
                    dataSource={baseShipmentTypeList}
                    loading={baseShipmentTypeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseShipmentType;
