...
import { getStatementOfAccountList,saveStatementOfAccount } from "@/api/settlement_center/cost_manage/statement_of_account_service";
...
const StatementOfAccount : React.FC = () => {
    ...
    // 对账单管理数据
    const [statementOfAccountList, setStatementOfAccountList] = useState([] as StatementOfAccountItemProps[]);
    ...
    const handleDelete = (record:StatementOfAccountItemProps) => {
        alert(record);
    };
    const handleEdit = (record:StatementOfAccountItemProps) => {
        const newData = statementOfAccountList.filter((item) => ` === `);
    ...
    const initFormData = {} as StatementOfAccountItemProps;
    const [formData, setFormData] = useState<StatementOfAccountItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveStatementOfAccount(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<StatementOfAccountItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='statement_of_account' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='statement_of_account' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='statement_of_account' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 对账单
                            <Tooltip
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>对账单</b></span>多笔账单的集合，用于分阶段核对，例如客户B的月结账单有50票工作单，您无法一天内核对完毕，需分3天完成，每天对账完成的账单生成对账单。
                                            </li>
                                        </ol>
                                    </div>
                                }
            ...
            <div className='nc-bill-table-area'>
                <Table<StatementOfAccountItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.StatementNumber}`}
                    showSorterTooltip={false}
                    dataSource={statementOfAccountList}
                    loading={statementOfAccountList.length === 0}
                    pagination={
                        {
                            size:'small',
