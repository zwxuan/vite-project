...
interface DetailModalProps {
    ...
    modalFlag: 'add' | 'edit';
    saving: boolean;
    formData: Partial<SummaryRuleItemProps>;
    onCancel: () => void;
...
const DetailModal: React.FC<DetailModalProps> = ({
    ...
    return (
        <Modal 
            open={open} 
            title={modalFlag === 'add' ? "新增分录摘要规则" : "编辑分录摘要规则"}
            onCancel={onCancel}
            destroyOnClose={true}
            maskClosable={false}
            closable={!saving}
        ...
        >
            <Form {...formItemLayout} style={{ maxWidth: 600 }} initialValues={formData} disabled={saving} onFinish={onOk}>
                        <Form.Item label="摘要规则ID" name="SummaryRuleCode" rules={[{ required: true, message: '' }]}>
                        ...
                        <Form.Item label="分组条件" name="GroupBy" rules={[{ required: true, message: '' }]}>
                            <Select style={{ flex: 1 }}
                                showSearch
