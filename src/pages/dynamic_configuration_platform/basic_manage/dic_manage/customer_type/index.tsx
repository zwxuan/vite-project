...
import { CustomerTypeItemProps } from "@/types/dynamic_configuration_platform/basic_manage/customer_type";
import { getCustomerTypeList,saveCustomerType } from "@/api/dynamic_configuration_platform/basic_manage/customer_type_service";
...
const CustomerType : React.FC = () => {
    ...
    // 客户分类数据
    const [customerTypeList, setCustomerTypeList] = useState([] as CustomerTypeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const customerTypeData = await getCustomerTypeList();
            // 设置客户分类台账数据
    ...
    const handleDelete = (record:CustomerTypeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:CustomerTypeItemProps) => {
        const newData = customerTypeList.filter((item) => `${item.Code}` === `${record.Code}`);
    ...
    const initFormData = {} as CustomerTypeItemProps;
    const [formData, setFormData] = useState<CustomerTypeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveCustomerType(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<CustomerTypeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='customer_type' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='customer_type' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='customer_type' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 客户分类
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<CustomerTypeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Code}`}
                    showSorterTooltip={false}
                    dataSource={customerTypeList}
                    loading={customerTypeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
