...
const TransportationLineTeuReport: React.FC = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const response = await fetch('/data/transportation_line_teu_data.json');
                if (!response.ok) {
    ...
    const s2DataConfig: S2DataConfig = {
        fields: {
            rows: ["route", "discharge_port"],
            columns: [],
            values: ["TEU", "HBL", "CBM", "RT"],
            valueInCols: true
        },
        meta: [
            {
    ...
    const s2Options: SheetComponentOptions = {
        ...
        totals: {
            row: {
                ...
                subTotalsDimensions: ["route", "discharge_port"],
            },
                ...
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{ marginLeft: "10px" }}>
                            <CustomIcon type="icon-Currency" style={{ color: 'red', fontSize: '24px' }} /> 航线货量分析
                        </span>
                    </div>
...
export default TransportationLineTeuReport;
