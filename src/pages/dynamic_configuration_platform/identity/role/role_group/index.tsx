...
const RoleGroup: React.FC = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const roleGroupData = await getRoleGroupList();
            // 设置角色组台账数据
            setRoleGroupList([...roleGroupData]);
        };
        getData();
    }, []);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveRoleGroup(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<RoleGroupItemProps> = {
                                ...
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>说明</b></span>角色组，是基于角色，按照业务场景进行分类管理，更科学的管理角色。
                                            </li>
                                        </ol>
                                    </div>
                                }
