...
import { getPaymentApplicationList,savePaymentApplication } from "@/api/settlement_center/cost_manage/payment_application_service";
...
const PaymentApplication : React.FC = () => {
    ...
    // 付款申请数据
    const [paymentApplicationList, setPaymentApplicationList] = useState([] as PaymentApplicationItemProps[]);
    const [uploadImportType,setUploadImportType] = useState(1);
    const navigate = useNavigate();
    // 获取付款申请数据
    useEffect(() => {
        const getData = async () => {
            const paymentApplicationData = await getPaymentApplicationList();
            // 设置付款申请台账数据
            setPaymentApplicationList([...paymentApplicationData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:PaymentApplicationItemProps) => {
        alert(record);
    };
    const handleEdit = (record:PaymentApplicationItemProps) => {
        const newData = paymentApplicationList.filter((item) => ` === `);
    ...
    const initFormData = {} as PaymentApplicationItemProps;
    const [formData, setFormData] = useState<PaymentApplicationItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await savePaymentApplication(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<PaymentApplicationItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='payment_application' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='payment_application' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='payment_application' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 付款申请
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<PaymentApplicationItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.PaymentNotificationNumber}`}
                    showSorterTooltip={false}
                    dataSource={paymentApplicationList}
                    loading={paymentApplicationList.length === 0}
                    pagination={
                        {
                            size:'small',
