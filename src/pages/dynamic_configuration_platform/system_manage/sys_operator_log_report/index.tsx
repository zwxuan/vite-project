...
import { getSysOperatorLogReportList,saveSysOperatorLogReport } from "@/api/dynamic_configuration_platform/system_manage/sys_operator_log_report_service";
...
const SysOperatorLogReport : React.FC = () => {

    // 操作日志统计数据
    const [sysOperatorLogReportList, setSysOperatorLogReportList] = useState([] as SysOperatorLogReportItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const sysOperatorLogReportData = await getSysOperatorLogReportList();
            // 设置操作日志统计台账数据
            setSysOperatorLogReportList([...sysOperatorLogReportData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:SysOperatorLogReportItemProps) => {
        alert(record);
    };
    const handleEdit = (record:SysOperatorLogReportItemProps) => {
        const newData = sysOperatorLogReportList.filter((item) => `${item.Department}` === `${record.Department}`);
    ...
    const initFormData = {} as SysOperatorLogReportItemProps;
    const [formData, setFormData] = useState<SysOperatorLogReportItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveSysOperatorLogReport(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<SysOperatorLogReportItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
                ...
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 操作日志统计
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<SysOperatorLogReportItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Department}`}
                    showSorterTooltip={false}
                    dataSource={sysOperatorLogReportList}
                    loading={sysOperatorLogReportList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default SysOperatorLogReport;
