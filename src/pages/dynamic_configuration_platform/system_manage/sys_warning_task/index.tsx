...
import { getSysWarningTaskList,saveSysWarningTask } from "@/api/dynamic_configuration_platform/system_manage/sys_warning_task_service";
...
const SysWarningTask : React.FC = () => {

    // 预警任务数据
    const [sysWarningTaskList, setSysWarningTaskList] = useState([] as SysWarningTaskItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const sysWarningTaskData = await getSysWarningTaskList();
            // 设置预警任务台账数据
            setSysWarningTaskList([...sysWarningTaskData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:SysWarningTaskItemProps) => {
        alert(record);
    };
    const handleEdit = (record:SysWarningTaskItemProps) => {
        navigate({
            pathname: '/warning_task/sys_warning_task/detail',
    ...
    const initFormData = {} as SysWarningTaskItemProps;
    const [formData, setFormData] = useState<SysWarningTaskItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveSysWarningTask(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<SysWarningTaskItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
                ...
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 预警任务
                        </span>
                    </div>
                    ...
                    <div style={{display: "flex"}}>
                        <div className="buttonGroup-component">
                            <div className="u-button-group">
                                ...
                                <Button type="primary" danger>监控</Button>
                            </div>
                        </div> 
            ...
            <div className='nc-bill-table-area'>
                <Table<SysWarningTaskItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.AppCode}`}
                    showSorterTooltip={false}
                    dataSource={sysWarningTaskList}
                    loading={sysWarningTaskList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
