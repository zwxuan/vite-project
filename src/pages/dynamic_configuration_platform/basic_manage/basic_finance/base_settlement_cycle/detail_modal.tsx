...
interface DetailModalProps {
    ...
    modalFlag: 'add' | 'edit';
    saving: boolean;
    formData: Partial<BaseSettlementCycleItemProps>;
    onCancel: () => void;
...
const DetailModal: React.FC<DetailModalProps> = ({
    ...
    return (
        <Modal 
            open={open} 
            title={modalFlag === 'add' ? "新增结算周期" : "编辑结算周期"}
            onCancel={onCancel}
            destroyOnClose={true}
            maskClosable={false}
            closable={!saving}
        ...
        >
            <Form {...formItemLayout} style={{ maxWidth: 600 }} initialValues={formData} disabled={saving} onFinish={onOk}>
                        ...
                        <Form.Item label="英文名称" name="EnName" rules={[{ required: true, message: '' }]}>
                            <Input onChange={onChange} />
                        </Form.Item>
                        <Form.Item label="中文名称" name="CnName" rules={[{ required: true, message: '' }]}>
                            <Input onChange={onChange} />
                        </Form.Item>
