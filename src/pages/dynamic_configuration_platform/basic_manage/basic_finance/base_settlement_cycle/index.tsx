...
import { getBaseSettlementCycleList,saveBaseSettlementCycle } from "@/api/dynamic_configuration_platform/basic_manage/base_settlement_cycle_service";
...
const BaseSettlementCycle : React.FC = () => {

    // 结算周期数据
    const [baseSettlementCycleList, setBaseSettlementCycleList] = useState([] as BaseSettlementCycleItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseSettlementCycleData = await getBaseSettlementCycleList();
            // 设置结算周期台账数据
            setBaseSettlementCycleList([...baseSettlementCycleData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseSettlementCycleItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseSettlementCycleItemProps) => {
        const newData = baseSettlementCycleList.filter((item) => `${item.Code}` === `${record.Code}`);
    ...
    const initFormData = {} as BaseSettlementCycleItemProps;
    const [formData, setFormData] = useState<BaseSettlementCycleItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseSettlementCycle(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseSettlementCycleItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_settlement_cycle' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_settlement_cycle' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_settlement_cycle' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 结算周期
                            <Tooltip
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>说明</b></span>结算周期（也称结算账期）是指从交易发生到完成支付的时间段。
                                            </li>
                                        </ol>
                                    </div>
                                }
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseSettlementCycleItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Code}`}
                    showSorterTooltip={false}
                    dataSource={baseSettlementCycleList}
                    loading={baseSettlementCycleList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BaseSettlementCycle;
