...
import { getBasePeriodicBillingList,saveBasePeriodicBilling } from "@/api/dynamic_configuration_platform/basic_manage/base_periodic_billing_service";
...
const BasePeriodicBilling : React.FC = () => {

    // 开票方式数据
    const [basePeriodicBillingList, setBasePeriodicBillingList] = useState([] as BasePeriodicBillingItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const basePeriodicBillingData = await getBasePeriodicBillingList();
            // 设置开票方式台账数据
            setBasePeriodicBillingList([...basePeriodicBillingData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BasePeriodicBillingItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BasePeriodicBillingItemProps) => {
        const newData = basePeriodicBillingList.filter((item) => `${item.Code}` === `${record.Code}`);
    ...
    const initFormData = {} as BasePeriodicBillingItemProps;
    const [formData, setFormData] = useState<BasePeriodicBillingItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBasePeriodicBilling(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BasePeriodicBillingItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_periodic_billing' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_periodic_billing' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_periodic_billing' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 开票方式
                            <Tooltip
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>说明</b></span>开具发票的周期。
                                            </li>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>选择建议</b></span>企业应根据客户类型、合同性质、现金流需求和内部管理效率选择合适的开票周期，并在合同中明确约定。
                                            </li>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>趋势</b></span>越来越多企业采用“月开 + 电子发票 + 自动对账”模式，提升效率与客户满意度。
                                            </li>
                                        </ol>
                                    </div>
                                }
            ...
            <div className='nc-bill-table-area'>
                <Table<BasePeriodicBillingItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Code}`}
                    showSorterTooltip={false}
                    dataSource={basePeriodicBillingList}
                    loading={basePeriodicBillingList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
...
export default BasePeriodicBilling;
