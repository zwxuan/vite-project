...
import { getRoleTagsList,saveRoleTags } from "@/api/dynamic_configuration_platform/identity/role_tags_service";
...
const RoleTags : React.FC = () => {

    // 角色标签数据
    const [roleTagsList, setRoleTagsList] = useState([] as RoleTagsItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const roleTagsData = await getRoleTagsList();
            // 设置角色标签台账数据
            setRoleTagsList([...roleTagsData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:RoleTagsItemProps) => {
        alert(record);
    };
    const handleEdit = (record:RoleTagsItemProps) => {
        const newData = roleTagsList.filter((item) => `${item.TagCode}` === `${record.TagCode}`);
    ...
    const initFormData = {} as RoleTagsItemProps;
    const [formData, setFormData] = useState<RoleTagsItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveRoleTags(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<RoleTagsItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='role_tags' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='role_tags' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='role_tags' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 角色标签
                            <Tooltip
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            <li style={{ marginBottom: '10px' }}><span style={{ marginRight: '10px', backgroundColor: '#f1f1f1', padding: '2px 10px' }}><b>说明</b></span>角色标签，是一种灵活的分类体系，方便更加灵活、精细化的管理角色；角色标签，仅限于对角色的多方位描述，不用于授权场景。
                                            </li>
                                        </ol>
                                    </div>
                                }
            ...
            <div className='nc-bill-table-area'>
                <Table<RoleTagsItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.TagCode}`}
                    showSorterTooltip={false}
                    dataSource={roleTagsList}
                    loading={roleTagsList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
