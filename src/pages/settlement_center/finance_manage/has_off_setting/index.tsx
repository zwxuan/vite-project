...
import { getHasOffSettingList,saveHasOffSetting } from "@/api/settlement_center/finance_manage/has_off_setting_service";
...
const HasOffSetting : React.FC = () => {

    // 已核销数据
    const [hasOffSettingList, setHasOffSettingList] = useState([] as HasOffSettingItemProps[]);
    ...
    const handleDelete = (record:HasOffSettingItemProps) => {
        alert(record);
    };
    const handleEdit = (record:HasOffSettingItemProps) => {
        const newData = hasOffSettingList.filter((item) => ` === `);
    ...
    const initFormData = {} as HasOffSettingItemProps;
    const [formData, setFormData] = useState<HasOffSettingItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveHasOffSetting(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<HasOffSettingItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='has_off_setting' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='has_off_setting' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='has_off_setting' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 已核销
                        </span>
                    </div>
                    ...
                    <div style={{display: "flex"}}>
                        <div className="buttonGroup-component">
                            <div className="u-button-group">
                                <Button type="primary" danger >修改核销</Button>
                                <Button type="primary" danger >撤销核销</Button>
                                <Button type="primary">凭证</Button>
                                <Button>查看明细</Button>
                                <Button>复核</Button>
                                <Button>取消复核</Button>
                                <Button>打印</Button>
            ...
            <div className='nc-bill-table-area'>
                <Table<HasOffSettingItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Writeoffserialnumber}`}
