...
import { EntryGroupingRuleItemProps } from "@/types/dynamic_onfiguration_platform/basic_manage/entry_grouping_rule";
import { getEntryGroupingRuleList,saveEntryGroupingRule } from "@/api/dynamic_onfiguration_platform/basic_manage/entry_grouping_rule_service";
...
const EntryGroupingRule : React.FC = () => {

    // 凭证分录规则数据
    const [entryGroupingRuleList, setEntryGroupingRuleList] = useState([] as EntryGroupingRuleItemProps[]);
    ...
    const handleDelete = (record:EntryGroupingRuleItemProps) => {
        alert(record);
    };
    const handleEdit = (record:EntryGroupingRuleItemProps) => {
        const newData = entryGroupingRuleList.filter((item) => `${item.EntryId}` === `${record.EntryId}`);
    ...
    const initFormData = {} as EntryGroupingRuleItemProps;
    const [formData, setFormData] = useState<EntryGroupingRuleItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveEntryGroupingRule(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<EntryGroupingRuleItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='entry_grouping_rule' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='entry_grouping_rule' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='entry_grouping_rule' />
            ...
            <div className='nc-bill-table-area'>
                <Table<EntryGroupingRuleItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.EntryId}`}
                    showSorterTooltip={false}
                    dataSource={entryGroupingRuleList}
                    loading={entryGroupingRuleList.length === 0}
                    pagination={
                        {
                            size:'small',
...
export default EntryGroupingRule;
