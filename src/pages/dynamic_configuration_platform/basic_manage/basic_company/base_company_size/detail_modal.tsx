...
interface DetailModalProps {
    ...
    modalFlag: 'add' | 'edit';
    saving: boolean;
    formData: Partial<BaseCompanySizeItemProps>;
    onCancel: () => void;
...
const DetailModal: React.FC<DetailModalProps> = ({
    ...
    return (
        <Modal 
            open={open} 
            title={modalFlag === 'add' ? "新增企业规模" : "编辑企业规模"}
            onCancel={onCancel}
            destroyOnClose={true}
            maskClosable={false}
            closable={!saving}
        ...
        >
            <Form {...formItemLayout} style={{ maxWidth: 600 }} initialValues={formData} disabled={saving} onFinish={onOk}>
                        <Form.Item label="企业规模编码" name="CompanySizeCode" rules={[{ required: true, message: '' }]}>
                            <Input onChange={onChange} />
                        </Form.Item>
                        <Form.Item label="企业规模名称" name="CompanySizeName" rules={[{ required: true, message: '' }]}>
                            <Input onChange={onChange} />
                        </Form.Item>
