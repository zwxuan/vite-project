...
import { getVisitCustomerList,saveVisitCustomer } from "@/api/dynamic_configuration_platform/basic_manage/visit_customer_service";
...
const VisitCustomer : React.FC = () => {

    // 拜访客户数据
    const [visitCustomerList, setVisitCustomerList] = useState([] as VisitCustomerItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const visitCustomerData = await getVisitCustomerList();
            // 设置拜访客户台账数据
            setVisitCustomerList([...visitCustomerData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:VisitCustomerItemProps) => {
        alert(record);
    };
    const handleEdit = (record:VisitCustomerItemProps) => {
        const newData = visitCustomerList.filter((item) => `${item.Id}` === `${record.Id}`);
    ...
    const initFormData = {} as VisitCustomerItemProps;
    const [formData, setFormData] = useState<VisitCustomerItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveVisitCustomer(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<VisitCustomerItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 120px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='visit_customer' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='visit_customer' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='visit_customer' />
            ...
            <div className='nc-bill-table-area'>
                <Table<VisitCustomerItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Id}`}
                    showSorterTooltip={false}
                    dataSource={visitCustomerList}
                    loading={visitCustomerList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
