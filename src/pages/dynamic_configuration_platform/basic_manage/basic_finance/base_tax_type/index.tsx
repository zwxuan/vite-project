...
import { getBaseTaxTypeList,saveBaseTaxType } from "@/api/dynamic_configuration_platform/basic_manage/base_tax_type_service";
...
const BaseTaxType : React.FC = () => {

    // 税种档案数据
    const [baseTaxTypeList, setBaseTaxTypeList] = useState([] as BaseTaxTypeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseTaxTypeData = await getBaseTaxTypeList();
            // 设置税种档案台账数据
            setBaseTaxTypeList([...baseTaxTypeData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseTaxTypeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseTaxTypeItemProps) => {
        const newData = baseTaxTypeList.filter((item) => `${item.TaxTypeCode}` === `${record.TaxTypeCode}`);
    ...
    const initFormData = {} as BaseTaxTypeItemProps;
    const [formData, setFormData] = useState<BaseTaxTypeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseTaxType(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseTaxTypeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 60px)', background: '#f9fbff'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_tax_type' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_tax_type' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_tax_type' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 税种档案
                            <Tooltip
                                title={
                                    <div className='rul_title_tooltip' style={{ backgroundColor: '#fff', color: '#000' }}>
                                        <ol style={{ color: '#666666', fontSize: '12px', paddingLeft: '2px' }}>
                                            ...
                                            本部分信息在税制档案信息的基础上进一步细化至税种信息，会成为税率档案的底层基础数据。
                                            </li>
                                        </ol>
                                    </div>
                                }
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseTaxTypeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.TaxTypeCode}`}
                    showSorterTooltip={false}
                    dataSource={baseTaxTypeList}
                    loading={baseTaxTypeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
