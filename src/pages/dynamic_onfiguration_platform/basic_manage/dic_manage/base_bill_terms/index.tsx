...
import { BaseBillTermsItemProps } from "@/types/dynamic_onfiguration_platform/basic_manage/base_bill_terms";
import { getBaseBillTermsList,saveBaseBillTerms } from "@/api/dynamic_onfiguration_platform/basic_manage/base_bill_terms_service";
...
const BaseBillTerms : React.FC = () => {

    // 提单条款数据
    const [baseBillTermsList, setBaseBillTermsList] = useState([] as BaseBillTermsItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseBillTermsData = await getBaseBillTermsList();
            // 设置提单条款台账数据
            setBaseBillTermsList([...baseBillTermsData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseBillTermsItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseBillTermsItemProps) => {
        const newData = baseBillTermsList.filter((item) => `${item.Code}` === `${record.Code}`);
    ...
    const initFormData = {} as BaseBillTermsItemProps;
    const [formData, setFormData] = useState<BaseBillTermsItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseBillTerms(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseBillTermsItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_bill_terms' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_bill_terms' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_bill_terms' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 提单条款
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseBillTermsItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Code}`}
                    showSorterTooltip={false}
                    dataSource={baseBillTermsList}
                    loading={baseBillTermsList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
