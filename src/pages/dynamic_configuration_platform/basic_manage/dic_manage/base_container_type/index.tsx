...
import { BaseContainerTypeItemProps } from "@/types/dynamic_configuration_platform/basic_manage/base_container_type";
import { getBaseContainerTypeList,saveBaseContainerType } from "@/api/dynamic_configuration_platform/basic_manage/base_container_type_service";
...
const BaseContainerType : React.FC = () => {

    // 箱型种类数据
    const [baseContainerTypeList, setBaseContainerTypeList] = useState([] as BaseContainerTypeItemProps[]);
    ...
    useEffect(() => {
        const getData = async () => {
            const baseContainerTypeData = await getBaseContainerTypeList();
            // 设置箱型种类台账数据
            setBaseContainerTypeList([...baseContainerTypeData]);
        };
        getData();
    }, []);
      
    const handleDelete = (record:BaseContainerTypeItemProps) => {
        alert(record);
    };
    const handleEdit = (record:BaseContainerTypeItemProps) => {
        const newData = baseContainerTypeList.filter((item) => `${item.Code}` === `${record.Code}`);
    ...
    const initFormData = {} as BaseContainerTypeItemProps;
    const [formData, setFormData] = useState<BaseContainerTypeItemProps>(initFormData);
    ...
    const handleOk = async () => {
        ...
        try {
            const response = await saveBaseContainerType(formData, (progress) => {
                // 更新通知中的进度条
                notification.open({
                    key,
                    message: '提示',
    ...
    //表格选中和取消时触发的函数
    const rowSelection: TableRowSelection<BaseContainerTypeItemProps> = {
    ...
    return (
        <div  style={{overflowY: 'auto',overflowX:'hidden', height: 'calc(100vh - 80px)'}}>
            ...
            <ModelExcelImport open={openExcel} onCancel={handleExcelCancel} businessType='base_container_type' importType={uploadImportType} />
            <ModelExcelImportTemplate open={openExcelTemplate} onCancel={handleExcelTemplateCancel}  businessType='base_container_type' />
            <ModelExcelImportTemplateUpdate open={openExcelTemplateUpdate} onCancel={handleExcelTemplateUpdateCancel}  businessType='base_container_type' />

            <div className="nc-bill-header-area">
                <div className="header-title-search-area">
                    <div className="BillHeadInfoWrap BillHeadInfoWrap-showBackBtn">
                        <span className="bill-info-title" style={{marginLeft: "10px"}}>
                            <CustomIcon type="icon-Currency"  style={{color:'red',fontSize:'24px'}} /> 箱型种类
                        </span>
                    </div>
            ...
            <div className='nc-bill-table-area'>
                <Table<BaseContainerTypeItemProps>
                    columns={columnsType}
                    rowSelection={{ ...rowSelection}}
                    rowKey={(record) => `${record.Code}`}
                    showSorterTooltip={false}
                    dataSource={baseContainerTypeList}
                    loading={baseContainerTypeList.length === 0}
                    pagination={{
                        size:'small',
                        pageSize:pageSize,
